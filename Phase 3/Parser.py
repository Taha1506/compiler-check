    ...
    def parse(self):
        while not self.scanner.is_end():
            ...
            while not action == 'shift':
                ...
                semantic_check = self.code_generator.semantic_analysis(current_grammar[0], self.token[1])
                if semantic_check:
                    self.semantic_errors.append(f'#{self.current_line + 1} : Semantic Error! ' + semantic_check)
                self.code_generator.generate(current_grammar[0], self.token[1])
